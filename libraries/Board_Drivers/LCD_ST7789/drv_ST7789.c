...
void LCD_Scan_Dir(uint8_t dir)
{
    ...
    if ((lcddev.dir == 1 && lcddev.id != 0X1963) ||
        (lcddev.dir == 0 && lcddev.id == 0X1963)) // 横屏时，对1963不改变扫描方向！竖屏时1963改变方向
    {
        switch (dir) // 方向转换
        {
            case 0:
    ...
    if (lcddev.id == 0x9341 || lcddev.id == 0X5310 || lcddev.id == 0X5510 ||
        lcddev.id == 0X1963) // 9341/5310/5510/1963,特殊处理
    {
        switch (dir)
        {
...
int drv_lcd_init(void)
{
    ...
    // 初始化完成以后,提速
    if (lcddev.id == 0x8552) // st7789v3可以设置WR时序为最快
    {
        // 重新配置写时序控制寄存器的时序
